...
class Node:
    ...
    def __init__(self, parent = None, dims = 0, reset_id = False, kernel_type = "rbf",
                 gamma_type = "auto", gpu_id = None, lb = None, ub = None):
    ...
    def propose_samples_turbo(self, num_samples, path, func, collect=False):
        return self.classifier.propose_samples_turbo(num_samples, path, func, collect=collect)
    ...
    def get_uct(self, Cp = 1.414):
        """
        Standard UCB1-Tuned formula for LaMCTS.

        Uses the tuned bound from Auer et al. (2002):
            UCB = x_bar + sqrt(2 * ln(parent.n) / n) * Cp

        For LaMCTS we multiply by Cp to control the explore/exploit trade-off:
            - Cp > 1 : stronger exploration (prefer less-visited nodes)
            - Cp < 1 : weaker exploration (prefer higher-reward nodes)
            - Cp ≈ 1.414 (sqrt(2)): standard UCB1 balance

        Cp is passed from MCTS and should be set adaptively during search
        (see adaptive_Cp in MCTS.dynamic_treeify).

        Note: x_bar is stored in MAXIMIZATION format (i.e. negated for
        minimization objectives), so higher x_bar = better region.
        """
        if self.parent is None:
            return float('inf')
        if self.n == 0:
            return float('inf')
        return self.x_bar + Cp * math.sqrt(2.0 * math.log(self.parent.n) / self.n)
